...
//    $this->setCacheDir($cacheDir);
		...
		public function deleteAllWithNoLifetime(): bool
		{
			...
			return IoHelper::deleteFiles(glob($this->getCachePath() . "*.$noLifetimeFileString.ekodb"));
		}
		...
		public function set(array $content)
		{
			...
			$noLifetimeFileString = self::NO_LIFETIME_FILE_STRING;
			$cacheFile = $cachePath . $token . ".$noLifetimeFileString.ekodb";
			
			if (is_int($lifetime)) {
				$cacheFile = $cachePath . $token . ".$lifetime.ekodb";
			}
			
			IoHelper::writeContentToFile($cacheFile, json_encode($content), IoHelper::getKey());
		}
		...
		public function get()
		{
			...
			$cacheFiles = glob($cachePath . $token . "*.ekodb");
			...
			if (!empty($cacheFile)) {
				...
				if (count($cacheParts) >= 3) {
					...
					if (is_numeric($lifetime)) {
						if ($lifetime === "0") {
							return json_decode(IoHelper::getFileContent($cacheFile, IoHelper::getKey()), true);
						}
		...
		{
			return IoHelper::deleteFiles(glob($this->getCachePath() . $this->getToken() . "*.ekodb"));
		}
