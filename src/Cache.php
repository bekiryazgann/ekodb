...
//    $this->setCacheDir($cacheDir);
  ...
  public function deleteAllWithNoLifetime(): bool
  {
    ...
    return IoHelper::deleteFiles(glob($this->getCachePath()."*.$noLifetimeFileString.so"));
  }
  ...
  public function set(array $content){
    ...
    $noLifetimeFileString = self::NO_LIFETIME_FILE_STRING;
    $cacheFile = $cachePath . $token . ".$noLifetimeFileString.so";

    if(is_int($lifetime)){
      $cacheFile = $cachePath . $token . ".$lifetime.so";
    }
  ...
  public function get(){
    ...
    $cacheFiles = glob($cachePath.$token."*.so");
  ...
  {
    return IoHelper::deleteFiles(glob($this->getCachePath().$this->getToken()."*.so"));
  }
