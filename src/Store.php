<?php
	...
	use Exception;
	use EkoDb\Classes\IoHelper;
	use EkoDb\Classes\NestedHelper;
	use EkoDb\Exceptions\InvalidArgumentException;
	use EkoDb\Exceptions\IdNotAllowedException;
	use EkoDb\Exceptions\InvalidConfigurationException;
	use EkoDb\Exceptions\IOException;
	use EkoDb\Exceptions\JsonException;
	...
	if (false === class_exists("\Composer\Autoload\ClassLoader")) {
		...
		foreach (glob(__DIR__ . '/*.php') as $class) {
			if (strpos($class, 'EkoDb.php') !== false || strpos($class, 'Store.php') !== false) {
				continue;
			}
			require_once $class;
		}
	...
	class Store
	{
		 ...
		 */
		public function __construct(string $storeName, string $databasePath, string $username, string $password)
		{
		 ...
		 * @throws InvalidConfigurationException
		 */
		public function changeStore(string $storeName, string $databasePath = null, string $username, string $password): Store
		{
			if (empty($databasePath)) {
				$databasePath = $this->getDatabasePath();
			}
			$this->__construct($storeName, $databasePath, $username, $username);
			return $this;
		}
		...
		public function getLastInsertedId(): int
		{
			...
			return (int)IoHelper::getFileContent($counterPath, $this->key);
		}
		...
		public function findById($id)
		{
			...
			$filePath = $this->getDataPath() . "$id.ekodb";
			
			try {
				$content = IoHelper::getFileContent($filePath, $this->key);
...
//        throw new InvalidArgumentException("Documents have to have the primary key \"$primaryKey\". Got data: $documentString");
			...
			$storePath = $this->getDataPath() . "$data[$primaryKey].ekodb";
			IoHelper::writeContentToFile($storePath, json_encode($data), $this->key);
...
//        throw new InvalidArgumentException("Documents have to have the primary key \"$primaryKey\". Got data: $documentString");
			...
			foreach ($data as $document) {
				// save to access file with primary key value because we secured it above
				$storePath = $this->getDataPath() . "$document[$primaryKey].ekodb";
				IoHelper::writeContentToFile($storePath, json_encode($document), $this->key);
			}
		...
		private function createStore()
		{
			...
			if (!file_exists($counterFile)) {
				IoHelper::writeContentToFile($counterFile, '0', $this->key);
			}
		}
		...
		private function writeNewDocumentToStore(array $storeData): array
		{
			...
			if (isset($storeData[$primaryKey])) {
				throw new IdNotAllowedException(
					"The \"$primaryKey\" index is reserved by EkoDb, please delete the $primaryKey key and try again"
				);
			}
			...
			IoHelper::writeContentToFile($filePath, $storableJSON, $this->key);
		...
		private function increaseCounterAndGetNextId(): int
		{
			...
			return (int)IoHelper::updateFileContent($counterPath, function ($counter) use ($dataPath) {
				...
				while (file_exists($dataPath . "$newCounter.ekodb") === true) {
					$newCounter++;
				}
