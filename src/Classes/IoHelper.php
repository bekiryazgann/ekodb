<?php
	...
	class IoHelper
	{
		 ...
		 * @throws IOException
		 */
		public static function getFileContent(string $filePath, int $key): string
		{
			...
			return self::decrypt($content, $key);
		}
		 ...
		 * @throws IOException
		 */
		public static function writeContentToFile(string $filePath, string $content, $key)
		{
			
			self::checkWrite($filePath);
			
			// Wait until it's unlocked, then write.
			if (file_put_contents($filePath, self::encrypt($content, $key), LOCK_EX) === false) {
		 ...
		 */
		public static function updateFileContent(string $filePath, Closure $updateContentFunction, int $key): string
		{
			self::checkRead($filePath);
			self::checkWrite($filePath);
			...
			$content = $updateContentFunction(self::decrypt($content, $key));
		...
		/**
		 * todo: All write and read functions bas64_encode int its place $this->encrypt()
		 * @param string $string
		 * @param int $key
		 * @return string
		 */
		public static function encrypt(string $string, int $key = 5): string {
			$result = '';
		...
		/**
		 * todo: All write and read functions bas64_decode int its place $this->decrypt()
		 * @param string $string
		 * @param int $key
		 * @return string
		 */
		public static function decrypt(string $string, int $key = 5): string{
			$result = "";
