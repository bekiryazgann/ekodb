	...
	class DocumentUpdater
	{
		...
		public function updateResults(array $results, array $updatable, bool $returnUpdatedDocuments)
		{
			...
			foreach ($results as $key => $data) {
				...
				$filePath = $dataPath . $primaryKeyValue . '.ekodb';
			...
			foreach ($results as $key => $data) {
				$filePath = $dataPath . $data[$primaryKey] . '.ekodb';
				...
				IoHelper::writeContentToFile($filePath, json_encode($data), IoHelper::getKey());
				$results[$key] = $data;
			}
		...
		public function removeFields(array &$results, array $fieldsToRemove)
		{
			...
			foreach ($results as &$document) {
				...
				$filePath = $dataPath . $document[$primaryKey] . '.ekodb';
				IoHelper::writeContentToFile($filePath, json_encode($document), IoHelper::getKey());
			}
			return $results;
		}
